...
export default function ConsumerOrdersPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in duration-500 min-h-screen py-8 px-4">

      {/* HEADER & SEARCH */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b pb-6">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 tracking-tight">My Orders</h1>
          <p className="text-gray-500 mt-1">Track and manage your waste material orders.</p>
        </div>
        ...
        <div className="relative w-full md:w-80">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 bg-white border text-black border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all shadow-sm"
          />
        </div>
      </div>
      ...
      <div className="space-y-4">
        {loading ? (
          <div className="flex flex-col gap-4">
            {[1, 2, 3].map((i) => (
              <div key={i} className="bg-white border rounded-xl p-6 h-32 animate-pulse" />
            ))}
          </div>
          ...
          filteredOrders.map((o) => (
            <div
              key={o.id}
              className="group bg-white border border-gray-200 rounded-xl p-6 hover:shadow-md transition-all duration-200 flex flex-col md:flex-row gap-6 relative overflow-hidden"
            >
              {/* IMAGE / ICON */}
              <div className="md:w-32 md:h-32 w-full h-48 bg-gray-100 rounded-lg shrink-0 overflow-hidden relative border border-gray-100">
              ...
              <div className="flex-1 space-y-3">
                <div className="flex justify-between items-start">
                  <div>
                    <h2 className="text-xl font-bold text-gray-900">{o.products?.name}</h2>
                    <p className="text-sm text-gray-500 flex items-center gap-1 mt-1">
                      Order ID: #{o.id.slice(0, 8)}
                    </p>
                  </div>
                  <span className={`px-3 py-1 rounded-full text-xs font-semibold border uppercase tracking-wide flex items-center gap-1 ${getStatusColor(o.status)}`}>
                    {o.status}
                  </span>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm mt-2">
                  <div className="p-3 bg-gray-50 rounded-lg border border-gray-100">
                    <p className="text-gray-500 text-xs uppercase font-semibold">Quantity</p>
                    <p className="font-medium text-gray-900 mt-0.5">{o.quantity} kg</p>
                  </div>
                  <div className="p-3 bg-gray-50 rounded-lg border border-gray-100">
                    <p className="text-gray-500 text-xs uppercase font-semibold">Total Cost</p>
                    <p className="font-medium text-gray-900 mt-0.5">₹{o.total_price.toLocaleString()}</p>
                  </div>
                </div>
              ...
              <div className="md:self-center">
                <Link
                  ...
                  className="w-full md:w-auto px-5 py-2.5 bg-white border border-gray-200 text-gray-700 font-medium rounded-lg hover:bg-gray-50 hover:border-gray-300 transition-colors flex items-center justify-center gap-2"
                >
